...
export default function Vendors({ vendors = [], inventoryItems = [], onDataChanged, userProfile }) {
    ...
    return (
        <div className="space-y-6 fade-in pb-20">
            ...
            {/* Vendor Grid - Mobile-friendly layout */}
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 sm:gap-4 p-2">
                {filteredVendors.map(vendor => {
                    ...
                    return (
                        <div
                            key={vendor.id}
                            className="relative aspect-[5/4] sm:aspect-square rounded-2xl overflow-hidden shadow-md bg-gradient-to-br from-emerald-900 to-emerald-950 cursor-pointer hover:shadow-lg transition-all border-2 border-lime-glow/30 hover:border-lime-glow/60"
                            onClick={async () => {
                        ...
                        >
                            ...
                            {/* Content */}
                            <div className="absolute inset-0 p-3 sm:p-4 pr-12 flex flex-col justify-between z-10">
                                {/* Top section - Vendor name and badges */}
                                <div>
                                    <h3 className="text-base sm:text-lg font-bold text-lime-glow truncate pr-10">{vendor.name}</h3>
                                    ...
                                    {(fabricThumbs.length > 0 || vendorFabricList.length > 0) && (
                                        <div className="flex gap-2 mt-2 sm:mt-3">
                                            {(fabricThumbs.length ? fabricThumbs : vendorFabricList.slice(0, 3)).map(f => (
                                                <div key={f.id} className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl overflow-hidden border border-lime-glow/40 shadow-sm bg-emerald-900/60 flex items-center justify-center text-[11px] font-bold text-white/80">
                                                    {f.imageUrl ? (
                            ...
                            <div className="absolute top-3 right-3 sm:top-2 sm:right-2 flex gap-1 z-20">
                                <button
                                    onClick={(e) => {
                                        e.stopPropagation()
